<template>
  <g>
    <g
      ...
      @mousedown="mouseDown"
    >
      <g v-for="(point, index) in points" @mousedown="mouseDownSegment($event, index)">
        <line
    ...
    <g v-else @mouseenter="mouseEnter" @mouseleave="mouseLeave" @mousedown="mouseDown">
      <g @mousedown="mouseDownSegment($event, 0)">
        ...
        <path :d="curve" style="stroke:#303133;" stroke-width="2" fill="none" />
      </g>
    </g>
    <DiagramPoint
...
export default {
  name: "DiagramLink",
  props: ["positionFrom", "positionTo", "id", "link", "index", "points"],
  ...
  methods: {
    mouseEnter() {
      ...
      this.$set(this.link, "selected", true);
    },
    mouseLeave() {
      this.largeStrokeStyle = "stroke:rgba(255,0,0,0.0);";
      this.$set(this.link, "selected", false);
    },
